...
import com.tc.tcsocket.config.JRedisPoolConfig; 
    	   ...
    	   /* ResourceBundle bundle = ResourceBundle.getBundle("redis");
            ...
            if (jedisPool == null) {
               ...
               config.setTestOnReturn(JRedisPoolConfig.TEST_ON_RETURN);
                int TIMEOUT = 10000;
                if (JRedisPoolConfig.REDIS_PASSWORD !=null && !JRedisPoolConfig.REDIS_PASSWORD.equals(""))
                {
                jedisPool = new JedisPool(config, JRedisPoolConfig.REDIS_IP, JRedisPoolConfig.REDIS_PORT,TIMEOUT,JRedisPoolConfig.REDIS_PASSWORD);
                log.info("JRedisPoolConfig.REDIS_PASSWORD:"+JRedisPoolConfig.REDIS_PASSWORD+"??");
                }else
                {
                	 log.info("JRedisPoolConfig.REDIS_PASSWORD is null");
                	jedisPool = new JedisPool(config, JRedisPoolConfig.REDIS_IP, JRedisPoolConfig.REDIS_PORT);
                }
               
                System.out.println("jedisPool init complete:" + config.getMaxIdle() +"  " +  config.getMaxTotal() + " " +config.getMaxWaitMillis());
            }
            else
            {
            	 System.out.println("jedispool exists!");
            	
            }
            	
            System.out.println("initJedisPool getMaxBorrowWaitTimeMillis:");
        }
